...
import { prdSchema, PRD_PRODUCT_NAME_MAX, PRD_DESCRIPTION_MIN, PRD_DESCRIPTION_MAX, PRD_TARGET_AUDIENCE_MAX } from "../prd";
import { analysisSchema, ANALYSIS_PRODUCT_NAME_MAX, ANALYSIS_INDUSTRY_MAX, ANALYSIS_COMPETITORS_MAX } from "../analysis";
import { contextSchema, CONTEXT_SECTION_MAX } from "../context";
import { multiAgentSchema, MULTI_AGENT_QUESTION_MIN, MULTI_AGENT_QUESTION_MAX } from "../multi-agent";
...
describe("prdSchema", () => {
  ...
  it("accepts valid input with targetAudience", () => {
    expect(prdSchema.safeParse({ ...valid, targetAudience: "Small businesses" }).success).toBe(true);
  });
  ...
  it("accepts productDescription at exact max length", () => {
    expect(prdSchema.safeParse({ ...valid, productDescription: "x".repeat(PRD_DESCRIPTION_MAX) }).success).toBe(true);
  });

  it("rejects productDescription above max length", () => {
    expect(prdSchema.safeParse({ ...valid, productDescription: "x".repeat(PRD_DESCRIPTION_MAX + 1) }).success).toBe(false);
  });

  it("accepts productName at exact max length", () => {
    expect(prdSchema.safeParse({ ...valid, productName: "x".repeat(PRD_PRODUCT_NAME_MAX) }).success).toBe(true);
  });

  it("rejects productName above max length", () => {
    expect(prdSchema.safeParse({ ...valid, productName: "x".repeat(PRD_PRODUCT_NAME_MAX + 1) }).success).toBe(false);
  });

  it("accepts targetAudience at exact max length", () => {
    expect(prdSchema.safeParse({ ...valid, targetAudience: "x".repeat(PRD_TARGET_AUDIENCE_MAX) }).success).toBe(true);
  });

  it("rejects targetAudience above max length", () => {
    expect(prdSchema.safeParse({ ...valid, targetAudience: "x".repeat(PRD_TARGET_AUDIENCE_MAX + 1) }).success).toBe(false);
  });

  it("accepts productDescription at exact min length", () => {
    expect(prdSchema.safeParse({ ...valid, productDescription: "x".repeat(PRD_DESCRIPTION_MIN) }).success).toBe(true);
  });
});
...
describe("analysisSchema", () => {
  ...
  it("accepts valid input with competitors", () => {
    expect(analysisSchema.safeParse({ ...valid, competitors: "Competitor A, B" }).success).toBe(true);
  });
  ...
  it("accepts productName at exact max length", () => {
    expect(analysisSchema.safeParse({ ...valid, productName: "x".repeat(ANALYSIS_PRODUCT_NAME_MAX) }).success).toBe(true);
  });

  it("rejects productName above max length", () => {
    expect(analysisSchema.safeParse({ ...valid, productName: "x".repeat(ANALYSIS_PRODUCT_NAME_MAX + 1) }).success).toBe(false);
  });

  it("accepts industry at exact max length", () => {
    expect(analysisSchema.safeParse({ ...valid, industry: "x".repeat(ANALYSIS_INDUSTRY_MAX) }).success).toBe(true);
  });

  it("rejects industry above max length", () => {
    expect(analysisSchema.safeParse({ ...valid, industry: "x".repeat(ANALYSIS_INDUSTRY_MAX + 1) }).success).toBe(false);
  });

  it("accepts competitors at exact max length", () => {
    expect(analysisSchema.safeParse({ ...valid, competitors: "x".repeat(ANALYSIS_COMPETITORS_MAX) }).success).toBe(true);
  });

  it("rejects competitors above max length", () => {
    expect(analysisSchema.safeParse({ ...valid, competitors: "x".repeat(ANALYSIS_COMPETITORS_MAX + 1) }).success).toBe(false);
  });
});
...
describe("contextSchema", () => {
  ...
  it("accepts section at exact max length", () => {
    expect(contextSchema.safeParse({ product_overview: "x".repeat(CONTEXT_SECTION_MAX) }).success).toBe(true);
  });
...
describe("multiAgentSchema", () => {
  ...
  it("trims question", () => {
    const r = multiAgentSchema.safeParse({ ...valid, question: "  Should we build a mobile app for B2B SaaS?  " });
    expect(r.success).toBe(true);
  ...
  it("accepts question at exact min length", () => {
    expect(multiAgentSchema.safeParse({ ...valid, question: "x".repeat(MULTI_AGENT_QUESTION_MIN) }).success).toBe(true);
  });

  it("accepts question at exact max length", () => {
    expect(multiAgentSchema.safeParse({ ...valid, question: "x".repeat(MULTI_AGENT_QUESTION_MAX) }).success).toBe(true);
  });

  it("rejects question above max length", () => {
    expect(multiAgentSchema.safeParse({ ...valid, question: "x".repeat(MULTI_AGENT_QUESTION_MAX + 1) }).success).toBe(false);
  });
});
