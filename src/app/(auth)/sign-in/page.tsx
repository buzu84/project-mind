...
const errorMessages: Record<string, string> = {
  auth_callback_error:
    "We could not verify your email link. Please request a new confirmation email or try signing in.",
...
function SignInForm() {
  ...
  const isFormValid = email.includes("@") && password.length >= AUTH_PASSWORD_MIN;
      ...
      {isDevMode() && (
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
          ...
          <p className="mb-3 text-xs text-amber-600">
            Skip auth and log in as <span className="font-medium">{DEV_USER.name}</span> (
            {DEV_USER.email})
          </p>
          <Button
            className="w-full"
    ...
    <main className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center">
          ...
          <p className="mt-1 text-sm text-gray-500">Log in to your ProductMind account</p>
        </div>
