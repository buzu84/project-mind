...
export function DecisionDetailClient({
  ...
  async function handleAnalyze() {
    setAnalyzing(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/decisions/${decision.id}/analyze`, {
        method: "POST",
      });
      if (!res.ok) {
  ...
  const isAnalysisStale =
    hasAnalysis &&
    recommendation !== null &&
    new Date(decision.updated_at).getTime() - new Date(recommendation.created_at).getTime() >
      STALE_BUFFER_MS;

  const analyzeCta = analyzing ? "Analyzing…" : hasAnalysis ? "Re-Analyze" : "Analyze Decision";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <h1
            ...
            tabIndex={-1}
            className="break-words text-2xl font-bold text-gray-900 focus:outline-none"
          >
            {decision.title}
          </h1>
        ...
        <div className="flex flex-shrink-0 flex-col items-start gap-2 sm:items-end">
          ...
          {!hasAnalysis && !analyzing && (
            <p className="max-w-[220px] text-right text-[11px] text-gray-400">
              Uses project context and uploaded evidence to generate options, assumptions, risks,
      ...
      {isAnalysisStale && !analyzing && (
        <div
          className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2 text-xs text-amber-700"
          role="status"
        >
          <svg
          ...
          <span>
            This decision was edited after the analysis was generated. Consider re-analyzing for
            up-to-date results.
          </span>
        </div>
      )}
      ...
      {analyzing && (
        <Card className="border-brand-200 bg-brand-50 py-8 text-center">
          <IconSparkles className="mx-auto h-7 w-7 animate-pulse text-brand-600" />
          <p className="mt-2 text-sm font-medium text-brand-900">Analyzing decision…</p>
          <p className="mt-1 text-xs text-brand-600">
            Retrieving project evidence, evaluating options, and generating a structured
            recommendation.
          </p>
        </Card>
      )}
      ...
      {decision.problem_statement && (
        <Card>
          <h3 className="mb-1 text-sm font-semibold text-gray-700">Problem Statement</h3>
      ...
      {decision.context_summary && (
        <Card>
          <h3 className="mb-1 text-sm font-semibold text-gray-700">Context</h3>
      ...
      {recommendation && (
        <Card className="border-blue-200 bg-blue-50/30">
          ...
          <p className="mb-3 text-sm text-gray-800">{recommendation.recommendation}</p>

          {recommendation.reasoning.length > 0 && (
            <div className="mb-3">
              <h4 className="mb-1 text-xs font-semibold uppercase text-gray-600">Reasoning</h4>
          ...
          {recommendation.next_validation_steps.length > 0 && (
            <div>
              <h4 className="mb-1 text-xs font-semibold uppercase text-gray-600">Next Steps</h4>
              <ul className="list-inside list-disc space-y-0.5 text-sm text-gray-700">
                {recommendation.next_validation_steps.map((s, i) => (
                  <li key={i}>{s}</li>
                ))}
              </ul>
            </div>
      ...
      {options.length > 0 && (
        <div>
          <h3 className="mb-3 text-base font-semibold text-gray-900">Decision Options</h3>
              ...
              <Card key={opt.id}>
                <div className="mb-2 flex items-start justify-between">
                  ...
                  <div className="flex flex-shrink-0 gap-1.5">
                    {opt.effort_estimate && (
                      <Badge variant="default">{opt.effort_estimate} effort</Badge>
                    )}
                    {opt.reversibility && opt.reversibility !== "unknown" && (
                      <Badge variant="default">{opt.reversibility} reversibility</Badge>
                    )}
                ...
                {opt.description && <p className="mb-2 text-xs text-gray-600">{opt.description}</p>}
            ...
            {assumptions.map((a) => (
              <Card key={a.id} className="py-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-shrink-0 gap-1.5">
                    {a.assumption_type && <Badge variant="default">{a.assumption_type}</Badge>}
                    <span
                      className={`inline-flex items-center rounded px-1.5 py-0.5 text-xs font-medium ${riskColors[a.risk_level] ?? ""}`}
                    >
                      {a.risk_level} risk
                    </span>
                ...
                {a.validation_method && (
                  <p className="mt-1 text-xs text-gray-500">Validation: {a.validation_method}</p>
                )}
              </Card>
            ))}
          </div>
      ...
      {evidenceLinks.length > 0 && (
        <div>
          <h3 className="mb-3 text-base font-semibold text-gray-900">Evidence</h3>
          <div className="space-y-2">
            {evidenceLinks.map((link) => (
              <Card key={link.id} className="py-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    {link.evidence.title && (
                      <p className="text-xs font-semibold text-gray-700">{link.evidence.title}</p>
                    )}
                    <p className="line-clamp-2 text-sm text-gray-600">{link.evidence.claim}</p>
                  </div>
                  <div className="flex flex-shrink-0 gap-1.5">
      ...
      {!hasAnalysis && !analyzing && (
        <Card className="flex flex-col items-center justify-center py-12 text-center">
          <IconSparkles className="mb-3 h-10 w-10 text-gray-300" />
          <p className="text-sm font-semibold text-gray-700">No analysis yet</p>
          <p className="mt-1 max-w-sm text-xs text-gray-500">
            Click &quot;Analyze Decision&quot; to generate AI-powered options, assumptions, risks,
            and a recommendation.
          </p>
        </Card>
      )}
