...
export default async function DecisionsPage({ params }: { params: { id: string } }) {
  ...
  const { data: decisions } = await supabase
    ...
    .select(
      "id, title, category, status, problem_statement, context_summary, confidence_score, created_at, updated_at, product_decision_recommendations(created_at)",
    )
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <DecisionsClient projectId={project.id} initialDecisions={enrichedDecisions} />
    </div>
  );
}
